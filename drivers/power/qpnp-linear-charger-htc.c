...
static int qpnp_lbc_bat_if_configure_btc(struct qpnp_lbc_chip *chip)
{
	...
	if (chip->cfg_btc_disabled || flag_keep_charge_on || flag_pa_recharge || chip->batt_disabled_by_sw) {
...
static int get_prop_batt_health(struct qpnp_lbc_chip *chip)
{
	...
	if (the_chip->batt_disabled_by_sw){
		temp = get_prop_batt_temp(chip);
		if (temp >= BATT_HOT_TEMP)
			return POWER_SUPPLY_HEALTH_OVERHEAT;
		if (temp <= BATT_COLD_TEMP)
...
int pm8916_charger_disable(bool enable)
{
	...
	if (!the_chip) {
		...
		enable = !!enable;
		rc = qpnp_lbc_charger_enable(the_chip, THERMAL, enable);
	}
...
bool pm8916_get_batt_sw_control(void)
{
	...
	if (!the_chip) {
		...
		rc = the_chip->batt_disabled_by_sw;
	}
	return rc;
}
...
int pm8916_mfg_get_charging_state(int *result)
{
	...
	vbat = get_prop_battery_voltage_now(the_chip)/1000;

	pr_info("fast_chg=%d,vbat=%d\n",fast_chg,vbat);

	if ((vbat >= MFG_CHG_VOL) && (fast_chg&BIT(5)))
		*result = 1;
	else
		*result = 0;
...
int pm8916_charger_get_attr_text(char *buf, int size)
{
	...
	len += scnprintf(buf + len, size - len, "BOOT_DONE: 0x%llX;\n", val);
	get_misc_reg((void *)MISC_TRIM3_REG, &val);
	len += scnprintf(buf + len, size - len, "TRIM3_REG: 0x%llX;\n", val);
	get_misc_reg((void *)MISC_TRIM4_REG, &val);
	len += scnprintf(buf + len, size - len, "TRIM4_REG: 0x%llX", val);
...
#ifdef ENABLE_GPIO_OVP_DET
static int _ovp_gpio = -1;
static int _ovp_irq = -1;
static irqreturn_t ovp_det_handler(int irq, void *dev)
{
	int value = 0;
	value = gpio_get_value(_ovp_gpio);
	if(!value){
		ovp = 1;
		htc_charger_event_notify(HTC_CHARGER_EVENT_OVP);
		pr_info("[BATT] %s OVP Happened , ovp = %d.\n", __func__, ovp);
	}

	else if(ovp){
		ovp = 0;
		htc_charger_event_notify(HTC_CHARGER_EVENT_OVP_RESOLVE);
		pr_info("[BATT] %s OVP Resolved , ovp = %d.\n", __func__, ovp);
	}
	return IRQ_HANDLED;
}

static int ovp_det_init(struct device *dev)
{
	int rc;
	_ovp_gpio = of_get_named_gpio(dev->of_node, "qcom,ovp_gpio", 0);

	if(gpio_is_valid(_ovp_gpio)){
		rc = gpio_request_one(_ovp_gpio, GPIOF_IN, "qcom,ovp_gpio");
		if(unlikely(rc)){
			pr_err("Failed to request GPIO %d, rc %d\n",_ovp_gpio, rc);
			return -1;
		}

		_ovp_irq = gpio_to_irq(_ovp_gpio);
		if(_ovp_irq < 0){
			pr_err("Unable to get irq number for GPIO %d, _ovp_irq %d\n",
					_ovp_gpio, _ovp_irq);
			goto fail;
		}
	}
	else{
		pr_err("invalid gpio from devtree\n");
		return -EINVAL;
	}
	rc = request_irq(_ovp_irq, ovp_det_handler, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, "ovp_det", dev);
	if(rc){
		pr_err("Unable to claim irq %d; rc %d\n",
				_ovp_irq, rc);
		goto fail;
	}
	pr_debug("%s: _ovp_gpio = %d, _ovp_irq = %d.\n", __func__, _ovp_gpio, _ovp_irq);
	return 0;
...
static void ovp_det_uninit(struct device *dev)
{
	free_irq(_ovp_irq, dev);
	if (gpio_is_valid(_ovp_gpio))
		gpio_free(_ovp_gpio);
}
#endif
...
static int qpnp_lbc_probe(struct spmi_device *spmi)
{
	...
	if (pm8916_bms_warmmax_by_batterydata())
		chip->cfg_warm_bat_mv = pm8916_bms_warmmax_by_batterydata();
	if (pm8916_bms_coolmax_by_batterydata())
		chip->cfg_cool_bat_mv = pm8916_bms_coolmax_by_batterydata();
	if (pm8916_bms_batt_disabled_by_sw())
		chip->batt_disabled_by_sw = pm8916_bms_batt_disabled_by_sw();
