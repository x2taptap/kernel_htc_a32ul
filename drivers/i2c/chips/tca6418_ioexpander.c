...
{
	int i;
	printk(KERN_INFO "%s()+, num:%d, private_ioexp_client = 0x%lx, addr:0x%x\n", __func__, num, (unsigned long)private_ioexp_client, private_ioexp_client->addr);
	...
	, const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "%s(), private_ioexp_client = 0x%lx\n", __func__, (unsigned long)private_ioexp_client);
